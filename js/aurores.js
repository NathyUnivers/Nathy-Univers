...
function dessinerVoile(yBase, amplitude, couleur1, couleur2, opacite, vitesse) {

    ctx.save();

    const respiration =
        Math.sin(temps * vitesse + yBase * 0.01) * amplitude;

    const derive =
        Math.sin(temps * (vitesse * 0.25) + yBase * 0.005) * 70;
    ...
    for (let x = -150; x <= canvas.width + 150; x += 8) {
        ...
        const y =
            Math.sin(x * 0.006 + temps * vitesse) * 28 +
            Math.sin(x * 0.013 - temps * vitesse * 0.6) * 14 +
            Math.sin(x * 0.022 + temps * vitesse * 0.3) * 8;
...
function dessiner() {
    ...
    // Voile secondaire (turquoise / argenté)
    dessinerVoile(
        canvas.height * 0.53,
        15,
        "180,245,255",
