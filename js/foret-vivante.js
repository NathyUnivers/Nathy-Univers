...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const positionsForetPC = [
        { x: 14, y: 72 }, { x: 22, y: 60 }, { x: 31, y: 44 },
        { x: 38, y: 68 }, { x: 46, y: 55 }, { x: 55, y: 37 },
        { x: 61, y: 63 }, { x: 69, y: 48 }, { x: 77, y: 69 },
        { x: 84, y: 56 }, { x: 89, y: 42 }
    ];

    const positionsForetMobile = [
        { x: 18, y: 22 }, { x: 30, y: 18 }, { x: 45, y: 24 },
        { x: 60, y: 20 }, { x: 75, y: 18 },
        { x: 22, y: 34 }, { x: 38, y: 38 },
        { x: 55, y: 36 }, { x: 70, y: 34 },
        { x: 48, y: 48 }, { x: 78, y: 46 }
    ...
    function creerLuciole(position, couleur, zone = "foret") {
        ...
        luciole.classList.add("firefly", couleur);
        ...
        luciole.style.animationDuration = (3 + Math.random() * 3) + "s";
    ...
    positionsForet
        ...
        .slice(0, 10)
        .forEach(position => creerLuciole(position, "gold"));

    margeGauche
        .sort(() => Math.random() - 0.5)
        .slice(0, 2)
        .forEach((position, index) =>
            creerLuciole(position, index === 0 ? "silver" : "blue", "marge")
        );
    ...
    margeDroite
        ...
        .slice(0, 3)
        .forEach((position, index) =>
            creerLuciole(position, index === 2 ? "blue" : "silver", "marge")
        );
